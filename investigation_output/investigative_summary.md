...
All five CSVs re-parse under Python's strict CSV reader with zero malformed rows: `master_tender_investigation.csv` at 1,155 data rows × 179 columns with every row carrying exactly 179 fields, `rule_deviations.csv` at 5,525 rows × 38 columns, `rules_broken_line_by_line.csv` at 1,583 rows × 38 columns, `bidder_detail.csv` at 1,189 rows × 20 columns, and `data_dictionary.csv` at 276 rows × 9. The master gained its rule-aggregate columns when the catalogue was applied and one further column, `portal_self_certified_signed_in_due_time`, when the award notices' own compliance field was read; every tender ID in the deviations file exists in the master, and no tender ID is duplicated in either. All five are UTF-8 with byte-order mark — verified byte-level — and fully quoted, so they open cleanly in Excel, Google Sheets and LibreOffice without a locale-dependent import step. Unavailable values are empty cells throughout; `data_dictionary.csv` carries one row per column per file explaining what an empty cell means there, and a sweep for leftover placeholder strings across all five files returns zero hits outside the columns where a negative verdict is itself the finding. The bidder file breaks down as 645 `AWARDED_BIDDER` rows, 335 `UNNAMED_REJECTED_BIDDERS_AGGREGATE` rows, 134 `JV_PARTNER_OF_WINNER` rows and 75 `DISCLOSED_BENEFICIAL_OWNER_OF_WINNER` rows; losing bidders appear as aggregates rather than names because the portal never prints their names, and inventing rows for them would have been fabrication.

Every citation in the broken-rule file was then made to resolve mechanically, which is the test that matters most: a reader must be able to open the named PDF at the named page and find the quoted words. A script re-opens each of the 1,583 lines' source notice and confirms the excerpt appears verbatim on the page recorded, and re-opens each distinct clause citation and confirms a quoted fragment appears on each page cited. Both now pass at 1,583 of 1,583 and 9 of 9. Getting there exposed five defects that all shared one cause — a page number recorded from the wrong frame of reference or a page cited without the quote that justified it.

Eighty-nine rows still carried a whole provenance map in the page column ("eligibility p.1") rather than a page, because the earlier repair only parsed strings containing a semicolon; the page is now parsed out of any non-numeric value with the original string preserved in `evidence_page_map`. Tender 248630's liquid-asset bar was quoted correctly but recorded on page 1 when the eligibility block runs onto page 2, so every row's excerpt is now re-located in its PDF and the page set to where the text actually is. Nineteen excerpt cells carried a mojibake apostrophe from extraction ("Bankâ??s Undertaking"), which would have made a reader's diff against the PDF look like a mismatch; the sequence is replaced with a straight apostrophe and the substitution recorded in the dictionary rather than done silently. R03 cited e-PG3A page 12 without quoting it, so ITT 5.1 — the baseline that the invitation is open to all potential tenderers save as the TDS specifies, which is precisely what an enlistment pre-condition narrows — is now quoted from that page.

The fifth was the most instructive. R01 cited e-PG3A pages "6, 36, 87" while its printed-page column read "s.1 p.6, p.28, p.79". PDF page 6 is part of the table of contents: the 6 was the *printed* page number left sitting in the PDF-page column. e-PG3A's front matter runs eight pages, so printed p.6 is PDF p.14 — and PDF p.14 carries ITT 5.14, the clause the row's own reference had named all along. Re-pointing the citation and quoting ITT 5.14 gives R01 the pair the finding needs: the tenderer's duty to supply beneficial-ownership information and consent to its publication, and the procuring entity's duty to publish it. The other two mappings in that row were already right at the same eight-page offset, which is how the error was isolated.
...
A third came out of the citation audit, and it is the one an editor should care about most, because it is the failure mode that gets a story killed rather than merely dented: R01, the largest single group of publishable findings at 63 lines, was citing e-PG3A page 6, which is a table-of-contents page. The number was the clause's *printed* page carried into the PDF-page column, an eight-page offset. Anyone checking the citation would have opened the contents list, found no clause, and reasonably concluded the finding was invented. The clause is real and sits on PDF page 14; it is now cited there and quoted from there. The same audit re-pointed one document page, filled one cited-but-unquoted page, normalised 89 page cells that held a whole provenance map, and repaired 19 mojibake apostrophes. None of these moved a verdict, and all of them would have cost credibility at the exact moment it is tested.

All five output files re-parse under Python's strict CSV reader with zero malformed rows. `master_tender_investigation.csv` is 1,155 rows by 179 columns with no duplicate tender IDs; `rule_deviations.csv` is 5,525 by 38; `rules_broken_line_by_line.csv` is 1,583 by 38, and every one of those 1,583 lines carries a verbatim clause quote, a page in the rule PDF, a quote from the tender or award document and a page in that document — zero exceptions on all four checks, and every excerpt has been re-found on the page it names; `bidder_detail.csv` is 1,189 by 20; `data_dictionary.csv` is 276 rows, one per column per file plus one describing itself. All five are UTF-8 with byte-order mark and fully quoted. A final sweep for leftover placeholder text across all five files returns zero hits outside the columns where a negative verdict is the finding. The whole battery is in `rule_scripts/verify_all.py`, which prints either the failing row or `ALL CHECKS PASSED`; it currently prints the latter, and it should be re-run after any future edit to any of the five files.
---
