                ...
                DOM.ledgerBody.innerHTML = State.filtered.slice(0, 150).map(record => {
                    ...
                    return `
                        <tr>
                            ...
                            <td><span class="risk-badge risk-${record.risk}">${record.risk}</span></td>
                            <td style="text-align:right; font-family:monospace;">৳ ${valueText}</td>
                            <td style="text-align:center; display:flex; justify-content:center; gap:8px; flex-wrap:wrap;">
                                <button class="btn-outline" onclick="App.openVerifyModal('${record.id}')">View / Verify</button>
