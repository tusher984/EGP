  ...
  section.innerHTML = `
    ...
    <div class="authority-map-layout" id="map-export">
      <figure class="authority-map-figure">
        <div id="map" role="img" aria-label="Map of Bangladesh showing district boundaries and corruption indicators for six development authorities"></div>
...
function mount() {
  ...
  if (!story) return;
  mounted = true;
  story.appendChild(authorityMapMarkup());
  import(`./map.js?authority-map=${Date.now()}`).catch(() => {
    const note = document.querySelector('#authority-map-report .map-note');
